...
from query_engine.intent_to_sql import resolve_user_question
from query_engine.sql_validator import validate_sql
from query_engine.query_runner import execute_query

from email_analyser.llm_gemini import call_llm
from email_analyser.prompts import (
    SQL_EXPLANATION_PROMPT,
    SQL_GENERATION_PROMPT
)

from error_handling import BaseAppError
...
def _handle_llm_sql(question):
    ...
    try:
        ...
        sql = call_llm(prompt, MODEL_SQL, 0)
...
def _generate_explanation(question, count_sql, list_sql, records):
    ...
    try:
        return call_llm(prompt, MODEL_EXPLAIN, 0)
    except Exception as e:
