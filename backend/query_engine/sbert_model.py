...
from backend.error_handling import BaseAppError, ModelLoadError
...
def load_questions(json_path):
    try:
        ...
        raise ModelLoadError(f"Failed to load dataset questions: {e}")
...
def load_model():
    try:
        ...
        raise ModelLoadError(f"Failed to load SBERT model: {e}")
...
def load_or_update_embeddings(model, current_questions):
    ...
    try:
        ...
        raise ModelLoadError(f"Embedding generation failed: {e}")
...
class SBERTMatcher:
    ...
    def __init__(self):
        ...
        try:
            ...
            raise ModelLoadError(f"SBERT initialization failed: {e}")
    ...
    def find_top_matches(self, query, top_k=3):
        ...
        try:
            ...
            raise ModelLoadError(f"SBERT matching failed: {e}")
